...
class controlController extends Controller {
	...
	public function action($serverid = null, $action = null) {
		...
		switch($action) {
			...
			case 'reinstall': {
				if($server['server_status'] == 1) {
					// Смена статуса на "Установка"
					$this->serversModel->updateServer($serverid, array('server_status' => 3));
					// Переустановка сервера
					$this->serversModel->sendServerAction($serverid, 'reinstall');
					// Смена статуса на "Выключен"
			...
			case 'delete': {
				if($server['server_status'] == 1) {
					$result = $this->serversModel->sendServerAction($serverid, 'delete');
					$result = $this->serversModel->parseServerOutput($result);
					if($result['status'] == "OK") {
			...
			case 'restart': {
				if($server['server_status'] == 2) {
					$result = $this->serversModel->sendServerAction($serverid, 'restart');
					$result = $this->serversModel->parseServerOutput($result);
					if($result['status'] == "OK") {
						$this->serversModel->updateServer($serverid, array('server_status' => 2));
						$this->data['status'] = "success";
			...
			case 'stop': {
				if($server['server_status'] == 2) {
					$result = $this->serversModel->sendServerAction($serverid, 'stop');
					$result = $this->serversModel->parseServerOutput($result);
					if($result['status'] == "OK") {
						$this->serversModel->updateServer($serverid, array('server_status' => 1));
						$this->data['status'] = "success";
	...
	public function ajax($serverid = null) {
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(!$errorPOST) {
				...
				$result = $this->serversModel->sendServerAction($serverid, 'updatepassword');
				$result = $this->serversModel->parseServerOutput($result);
				if($result['status'] == "OK") {
